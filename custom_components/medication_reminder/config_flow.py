...
from .const import CONF_DOSES, CONF_MEDS, CONF_PATIENT, CONF_TIME, DOMAIN
...
class MedicationReminderConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        """Ask for the patient's name and create the entry."""
        errors: dict[str, str] = {}
        ...
        return self.async_show_form(
            step_id="user",
            data_schema=vol.Schema({vol.Required(CONF_PATIENT): str}),
            errors=errors,
        )
...
class MedicationReminderOptionsFlow(config_entries.OptionsFlow):
    """Add or remove doses for a patient."""
    ...
    async def async_step_init(
        ...
        return self.async_show_menu(
            step_id="init", menu_options=["add_dose", "remove_dose"]
        )
    ...
    async def async_step_add_dose(
        ...
        if user_input is not None:
            ...
            return self.async_create_entry(title="", data={CONF_DOSES: doses})
        schema = vol.Schema(
            {
    ...
    async def async_step_remove_dose(
        ...
        if user_input is not None:
            ...
            kept = [d for i, d in enumerate(doses) if str(i) not in remove]
            return self.async_create_entry(title="", data={CONF_DOSES: kept})
        choices = {
